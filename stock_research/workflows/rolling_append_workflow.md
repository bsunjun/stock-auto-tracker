...
5. **templates is dry-run only** — `stock_research/templates/*.csv` 는 헤더/스키마 검증용. `--apply` 가 templates 경로를 가리키면 `rolling_append.py` 가 거부한다 (exit 2). 실제 누적은 항상 Drive 의 `_rolling/` 경로에서 수행한다.
...
`stock_research/templates/*.csv` 는 **dry-run 전용** (header/schema 검증).
실제 `--apply` 는 Drive 의 `_rolling/` 경로 (저장소 바깥) 에서 수행한다.

```
# 1) Dry-run against the in-repo template (header schema check)
...
# 2) Apply against the real rolling csv (outside templates/)
python stock_research/scripts/rolling_append.py \
  --csv "$STOCK_RESEARCH_ROOT/03_daily_reports/_rolling/candidate_history.csv" \
  --rows /path/outside/repo/candidate_rows.json \
  --dedupe-keys date,ticker,source_key \
  --apply
```

> `--apply` 가 `stock_research/templates/...` 경로를 가리키면 다음과 같이 거부된다:
> ```
...
```
# 1) Dry-run with strict-estimate (templates/ allowed: dry-run only)
python stock_research/scripts/rolling_append.py \
...
# 2) Apply against the real rolling csv (NOT templates/)
python stock_research/scripts/rolling_append.py \
...
## Where History Lives
- repo: 헤더만 있는 템플릿 (이 디렉토리). dry-run / 스키마 검증 전용.
- Drive: 실제 누적 데이터 (`$STOCK_RESEARCH_ROOT/03_daily_reports/_rolling/...`). repo의 템플릿 파일에는 데이터를 직접 커밋하지 않는다.
- 실험적 append 결과를 repo에 푸시하지 말 것 — `git status`에 `stock_research/templates/*.csv`가 잡히면 반드시 검토 후 되돌릴 것.
- `rolling_append.py` 는 `--apply` 가 `stock_research/templates/...` 경로를 가리키면 exit 2 로 거부한다 (PR #8 hardening). dry-run 은 그대로 허용된다.
