# PR #61 Acceptance Criteria

This document lists the acceptance criteria that PR #61 must satisfy
before it is merged.
...
- [ ] All seven new files reside under the
      `read_only_observation_approval_packet/` directory and
      nowhere else.
- [ ] No file outside `read_only_observation_approval_packet/` is
      created or modified.

## Non-Modification Criteria

- [ ] No file from PR #50 – PR #60 is modified.
...
- [ ] No real price, volume, or order-book value appears in any
      added file.
...
- [ ] No real approval record is committed. The approval record
      template in this PR contains no real values.
...
- [ ] The required approval wording appears verbatim once in
      `APPROVAL_WORDING_REQUIREMENTS.md` as the prescribed required
      wording. It is not asserted as approval anywhere else.
- [ ] The forbidden approval phrases are listed only in the
      "Forbidden Approval Wording" section of
      `APPROVAL_WORDING_REQUIREMENTS.md`, and each is presented as
...
established in PR #50 – PR #60 must not appear in any added file
...
documents established in PR #50 – PR #60.
...
- [ ] The merge of PR #61 alone does **not** start real-data
      observation.
