# PR #59 Acceptance Criteria

This document lists the acceptance criteria that PR #59 must satisfy
before it is merged.
...
- [ ] All seven new files reside under the
      `real_data_boundary_planning/` directory and nowhere else.
- [ ] No file outside `real_data_boundary_planning/` is created or
      modified.

## Non-Modification Criteria

- [ ] No file from PR #50 – PR #58 is modified.
...
- [ ] No real price, volume, or order book value appears in any added
      file.
- [ ] No real news headline, article body, or attribution appears.
- [ ] No real disclosure identifier (for example, no DART
      acceptance number, no KIND notice id) appears.
...
- [ ] No occurrence of the `direct_trade_signal` flag set truthy in
      any added file.
- [ ] No occurrence of the `trade_signal` field set truthy or set to
      a numeric `1` literal in any added file.
...
established in PR #50 – PR #58 must not appear in any added file
(count = 0). The categories are referenced by description only and are
not reproduced literally in this document:
...
For the literal phrase list, see the corresponding doctrine documents
established in PR #50 – PR #58.
...
- [ ] The merge of PR #59 alone does **not** start the real-data
      stage.
...
- `operator_decision` field set to the execute literal: 0 occurrences
