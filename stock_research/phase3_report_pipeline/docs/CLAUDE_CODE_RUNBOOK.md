...
- 목표주가-only 리포트는 `structured_extraction.json` 에 들어가지 않는다.
  `target_price_secondary.json` (audit-only) 에만 기록된다.
...
- `target_price_secondary.json` — audit-only (목표주가 변경만 있는 PDF)
