...
on **observed price/volume behavior** ("price-action-first"), with growth and
fundamentals serving as a *gate* and a *narrative*, never as a substitute for
tape evidence.
...
| Role of growth | The thesis | A gate and a tailwind |
...
2. **Growth Gate.** A name must clear a growth floor (operational, not
   forecast) to be eligible. Eligibility is binary; it does not size the
   position.
...
| Domain | Reference |
|---|---|
| Growth eligibility | `PBKR_V4_GROWTH_POLICY.md` |
